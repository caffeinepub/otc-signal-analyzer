// Technical Analysis Engine — DeepSeek OTC AI v12 (World-Class AI Accuracy)
// Uses 17 indicators: adds Parabolic SAR, Fibonacci Retracement, ATR volatility filter,
// signal persistence, triple-confirmation boost, MTF anti-veto, and consensus gate.
...
export function generateSignal(
  ...
  const prices = priceHistory ?? generatePriceHistory(pair, 200);
  const now = new Date();
  ...
  // Determine leading direction so far
  const leadingBull = bullishWeight >= bearishWeight;
  if (leadingBull && mtfTrend.signal === "SELL") {
    // MTF opposes the leading direction — halve its weight contribution
    bearishWeight -= 2.5 * 0.5; // remove half of MTF's weight from bear side
  ...
  // If MACD + EMA + MTF all agree on the winning direction, boost by 35%
  const tripleUp =
  ...
  const signalStrength: "STRONG" | "MODERATE" | "WEAK" =
    ratio >= 0.68 ? "STRONG" : ratio >= 0.58 ? "MODERATE" : "WEAK";
