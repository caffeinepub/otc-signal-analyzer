...
import type { PriceData as _PriceData, Result as _Result, Result_1 as _Result_1 } from "./declarations/backend.did.d.ts";
export class Backend implements backendInterface {
    ...
    async getLastPrices(): Promise<Result_1> {
        if (this.processError) {
            try {
                const result = await this.actor.getLastPrices();
    ...
    async getLivePrices(): Promise<Result> {
        if (this.processError) {
            try {
                const result = await this.actor.getLivePrices();
...
function from_candid_opt_n3(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [_PriceData]): PriceData | null {
...
function from_candid_variant_n2(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: {
    ...
    return "ok" in value ? {
        __kind__: "ok",
        ok: from_candid_opt_n3(_uploadFile, _downloadFile, value.ok)
    } : "err" in value ? {
        __kind__: "err",
        err: value.err
    } : value;
